    ...
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden grid-pattern">
      ...
      <motion.div style={{ y }} className="absolute inset-0 pointer-events-none">
        ...
        <div
          ...
          style={{ background: "radial-gradient(ellipse 80% 60% at 50% 40%, rgba(249,115,22,0.07) 0%, transparent 70%)" }}
        />
      </motion.div>
      ...
      >
        {/* Badge */}
        {/* NOTE: above-fold elements use opacity:1 as initial so the browser
            paints them immediately — this is the key LCP optimization.
            Visual animation still runs (y transform), just no invisible flash. */}
        <motion.div
          initial={{ y: 20 }}
          animate={{ y: 0 }}
        ...
        {/* Headings — h1 is the LCP element; must never start at opacity:0 */}
        <motion.h1
          initial={{ y: 30 }}
          animate={{ y: 0 }}
        ...
        >
          {t("descPrefix")}{" "}
          <span className="text-orange-400 font-semibold">{t("descHighlight")}</span>{" "}
          {t("descSuffix")}
        </motion.p>
        ...
        >
          {[
            { label: t("statProjects"), value: "20+" },
            { label: t("statCities"), value: "5+" },
            { label: t("statSatisfaction"), value: "100%" },
          ].map((stat, i) => (
