...
type PortfolioProject = {
  ...
  name: string;
  slug: string;
  category: string; // "installation" | "event" | "videotron"
  location: string;
  year: string;
  tag: string;
  tagEN?: string;
  images: string[]; // Sanity CDN URLs (auto AVIF/WebP)
  color: string;
  gradient: string;
};
...
function PortfolioCard({ project, i }: { project: PortfolioProject; i: number }) {
  ...
  return (
    <motion.div
      variants={scaleIn}
      className={`group relative overflow-hidden rounded-3xl border border-white/8 cursor-pointer card-hover ${
        i === 0 ? "md:col-span-2" : ""
      }`}
    ...
    >
      ...
      <div
        className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 rounded-full opacity-20 transition-opacity duration-500 group-hover:opacity-40"
        style={{
              ...
              {images.map((_, idx) => (
                <button
                  ...
                  style={{
                    ...
                    transform:
                      idx === activeIdx ? "scale(1.3)" : "scale(1)",
                  }}
                />
              ))}
            </div>
...
export function PortfolioGrid({
  ...
  const filtered = (() => {
    if (activeTab === "videotron")
      return projects.filter((p) => p.category === "videotron");
    if (activeTab === "led") {
      const led = projects.filter(
        (p) => p.category === "installation" || p.category === "event"
      );
      if (activeLedSub === "installation")
        return led.filter((p) => p.category === "installation");
      if (activeLedSub === "event")
        return led.filter((p) => p.category === "event");
      return led;
    }
    return projects;
  })();
        ...
        <motion.div variants={stagger} className="grid md:grid-cols-2 gap-6">
          {filtered.length === 0 ? (
            ...
            filtered.map((project, i) => (
              <PortfolioCard key={project._id} project={project} i={i} />
            ))
          )}
        </motion.div>
      </AnimatedSection>
